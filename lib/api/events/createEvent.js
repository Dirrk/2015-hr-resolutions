...
module.exports = function createEvent(request, reply) {
    var doc = { event_details: {} },
        ...
        geocoder.geocode({address: input.address, zipcode: input.zip, country: 'US'}, function(err, res) {
            ...
            doc.event_details.volunteers_needed = {
                min: input.min_volunteers,
                max: input.max_volunteers
            };

            self.els.index({index: config.elsEventsIndex, type: config.elsEventsType, id: doc.event_details.id, body: doc}, function (err, resp) {
                if (err) {
