...
module.exports = function getByLocation(request, reply) {
    ...
    if (request.query.search) {
        body.query = {
            multi_match: {
                ...
                fields: ['event_details.name', 'event_details.description', 'event_details.items_needed.name', 'event_details.items_needed.description', 'event_details.location.city']
            }
        };
    }
    ...
    this.els.search(
        {
            index: config.elsDonationsIndex,
            type: config.elsDonationsType,
            size: 10,
            from: startAt,
